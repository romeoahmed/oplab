import { initialState } from '$lib/workbench/load/initial-state';
...
test('incomplete and overflowing setup values never reach the wire', () => {
  for (const value of ['', '-1', '+1', '1.5', '1e3', '0b10', '0x', '0x10000000000000000']) {
    expect(() => initialState({ registers: [{ name: 'rax', value }], mappings: [] })).toThrow(
      RangeError,
    );
  }
  for (const [length, flags] of [
    ...
    expect(() =>
      initialState({ registers: [], mappings: [{ address: '0x8000', length, flags }] }),
    ).toThrow(RangeError);
  }
  expect(() =>
    initialState({
