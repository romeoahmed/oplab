<script lang="ts">
  ...
  import { Binary, Box, ChevronDown, FileOutput, FileInput, FolderOpen } from '@lucide/svelte';
...
<DropdownMenu.Root>
  ...
  <DropdownMenu.Portal>
    <DropdownMenu.Content collisionPadding={12} class="command-menu" align="start" sideOffset={6}>
      <DropdownMenu.Item
        onSelect={() => {
          open('source', m.import_source({}, options));
        }}
      >
        <FileInput size={16} aria-hidden="true" />{m.import_source({}, options)}
      </DropdownMenu.Item>
      <DropdownMenu.Item
        onSelect={() => {
      ...
      >
        <FileOutput size={16} aria-hidden="true" />{m.export_source({}, options)}
      </DropdownMenu.Item>
      ...
      >
        <FileOutput size={16} aria-hidden="true" />{m.export_binary({}, options)}
      </DropdownMenu.Item>
      <DropdownMenu.Separator />
      <DropdownMenu.Item
      ...
      >
        <FileOutput size={16} aria-hidden="true" />{m.export_image({}, options)}
      </DropdownMenu.Item>
