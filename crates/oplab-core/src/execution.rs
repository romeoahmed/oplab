...
impl ExecutionState {
    ...
    ///
    /// # Errors
    /// Returns `Transition` when the event is illegal in the current state.
...
impl Generation {
    ...
    /// Advance after a successful load/reset, invalidating prior observations.
    ///
    /// # Errors
    /// Returns `CounterExhausted` rather than reusing an identifier.
