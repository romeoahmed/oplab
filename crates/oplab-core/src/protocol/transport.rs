...
///
/// # Errors
/// Rejects oversized output and propagates pipe failures.
...
/// Serialize a response into a bounded buffer before emitting any frame bytes.
///
/// # Errors
/// Returns `OutputLimit` without partial output when serialization exceeds its budget.
...
/// Encode the same bounded reply for framed workers and newline-delimited CLI output.
///
/// # Errors
/// Returns `OutputLimit` if the control budget would be exceeded.
...
///
/// # Errors
/// Validates all lengths and metadata before writing; pipe failure ends the connection.
...
/// Read a complete response without treating partial binary transfers as success.
///
/// # Errors
/// Rejects invalid lengths, wrong frame kinds, unexpected chunk sizes, and truncated EOF.
...
/// Read an ordered control request and any declared ELF image.
///
/// # Errors
/// Rejects malformed metadata, invalid lengths, wrong kinds, and partial transfers.
...
///
/// # Errors
/// Rejects mismatched payloads before writing any bytes and propagates pipe failures.
...
///
/// # Errors
/// Rejects empty or oversized lengths, incorrect frames, and partial transfers.
...
///
/// # Errors
/// Rejects invalid lengths before writing and propagates pipe failures.
pub fn write_stream(
    writer: &mut impl Write,
    event: &StreamEvent,
    memory: Option<&[u8]>,
...
/// Read either a correlated reply or an unsolicited event, including binary bytes.
///
/// # Errors
/// Rejects malformed metadata, wrong frame kinds, and partial transfers.
