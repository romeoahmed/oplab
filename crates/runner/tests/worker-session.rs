...
fn settle(client: &mut Client, key: SessionKey) -> TestResult<Message> {
    ...
    while Instant::now() < deadline {
        let message = client.request_until(
            Command::Execute {
                session: key,
                action: SessionAction::Observe { memory: None },
            },
            None,
            deadline,
        )?;
...
fn stream_until(
    ...
    let deadline = Instant::now() + Duration::from_secs(5);
    while Instant::now() < deadline {
        let message = client.event_until(deadline)?;
...
fn simd_observations_keep_all_bits_across_worker_framing_and_reset() -> TestResult {
    for (target, length, code) in [
        (Target::X86_64, 32, "vmovdqu ymm15, [rip + output]"),
        (
            Target::Aarch64,
            256,
            ".arch armv9-a+sve2\nadr x0, output\nldr z31, [x0]",
        ...
        interactive::worker(|client| {
            ...
            let last = match registers {
                Registers::X86_64 { ymm, .. } => &ymm[15],
                Registers::Aarch64 { z, .. } => &z[31],
            };
            assert_eq!(last.as_le_bytes(), value);
            assert_eq!(captured.payloads, vec![value.clone()]);
...
#[test]
fn vector_writes_merge_current_lanes_and_reject_stale_generations() -> TestResult {
    use oplab_core::{protocol::execution::VectorWrite, registers::VectorBits};
    for (target, name, source) in [
        (
            Target::X86_64,
            "xmm15",
            "movdqu [rip + output], xmm15\ndone: nop\n.data\noutput: .zero 16",
        ),
        (
            Target::Aarch64,
            "v31",
        ...
        interactive::worker(|client| {
            client.request(Command::Hello { version: VERSION }, None)?;
            let loaded = load(client, target, &image, completion, 20, None)?;
            let initial = observed(&loaded)?;
            let key = initial.key;
            let write = |width: u16, lane, value: u128| -> TestResult<SessionAction> {
                let length = usize::from(width.div_ceil(8))
                    .max(usize::try_from((128 - value.leading_zeros()).div_ceil(8))?);
                Ok(SessionAction::WriteVector(VectorWrite {
                    name: name.into(),
                    width,
                    lane,
                    value: VectorBits::new(value.to_le_bytes()[..length].to_vec())?,
                }))
            };
            execute(client, key, write(128, 0, u128::MAX)?)?;
            execute(client, key, write(64, 0, 0x0123_4567_89ab_cdef)?)?;
            let changed = execute(client, key, write(32, 3, 0x7fc1_2345)?)?;
            let expected = 0x7fc1_2345_ffff_ffff_0123_4567_89ab_cdef_u128;
            let bank = observed(&changed)?
                .registers
                .as_ref()
                .ok_or("missing bank")?;
            ...
            assert_eq!(&value.as_le_bytes()[..16], &expected.to_le_bytes());
            let invalid = execute(client, key, write(8, 0, 256)?)?;
            assert!(
                matches!(invalid.response.result, Reply::Error(error) if error.code == DiagnosticCode::InvalidInput)
            );
            let retained = execute(client, key, SessionAction::Observe { memory: None })?;
            assert_eq!(
                observed(&retained)?.registers,
                observed(&changed)?.registers
            );
            let rounded = execute(
                client,
                key,
                SessionAction::SetRounding(oplab_core::registers::RoundingMode::Up),
            )?;
            match observed(&rounded)?
                .registers
                .as_ref()
                .ok_or("missing bank")?
            {
                Registers::X86_64 { mxcsr, .. } => assert_eq!(*mxcsr, 0x5f80),
                Registers::Aarch64 { fpcr, .. } => assert_eq!(*fpcr, 0x0040_0000),
            }
            execute(client, key, SessionAction::Run)?;
            let done = settle(client, key)?;
            assert_eq!(
            ...
            assert_eq!(stored.payloads, vec![expected.to_le_bytes().to_vec()]);
            let reset = execute(client, key, SessionAction::Reset)?;
            assert_eq!(observed(&reset)?.registers, initial.registers);
            let stale = execute(client, key, write(128, 0, u128::MAX)?)?;
            assert!(
                matches!(stale.response.result, Reply::Error(error) if error.code == DiagnosticCode::StaleSession)
            );
            ...
            assert_eq!(observed(&unchanged)?.registers, initial.registers);
            client.request(Command::Shutdown, None)?;
            Ok(())
        })?;
    }
