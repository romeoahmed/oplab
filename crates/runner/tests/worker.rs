...
use oplab_runner::worker::{Worker, WorkerError};
mod common;
            ...
            Request {
                ...
                command: Command::Assemble {
                    identity: BuildIdentity {
                        ...
                        assembler: oplab_toolchain::assembly::identity(),
                    },
                    source: source.into(),
                },
            }
...
fn real_worker_process_handshakes_and_exits_on_shutdown() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    for _ in 0..2 {
        ...
        assert!(received.insert(id), "duplicate assembly reply");
        let expected = match id {
            2 => vec![0x90],
            3 => vec![0x1f, 0x20, 0x03, 0xd5],
            _ => return Err("unexpected assembly reply".into()),
        };
