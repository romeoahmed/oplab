...
impl Worker {
    /// Dispatch one request. Only a matching first handshake unlocks work.
    ///
    /// # Errors
    /// Reused/out-of-order identifiers or requests after shutdown are connection failures.
    pub fn handle(
        &mut self,
...
/// Embedders use the engine operations or `Worker` for synchronous dispatch instead.
///
/// # Errors
