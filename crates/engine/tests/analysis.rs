...
use oplab_engine::decode::{DecodeError, analyze};
...
fn implicit_stack_conditional_access_and_undefined_flags_are_distinct() -> Result<(), DecodeError> {
    ...
    let xor = x86(&[0x31, 0xc0])?; // XOR EAX,EAX zeroes RAX without reading it.
    assert_eq!(xor.registers, [register("rax", DataAccess::Write)]);
    ...
    assert!(flags.set.contains(&"ZF".into()));
    ...
    let save = x86(&[0x0f, 0xae, 0x00])?; // FXSAVE [RAX]
    assert!(matches!(
        save.architecture,
        ArchitectureAnalysis::X86 {
            registers_incomplete: true,
            ..
        }
    ));
    Ok(())
...
-> Result<(), Box<dyn std::error::Error>> {
    for (target, source, bytes, feature) in [
        ...
        let object = oplab_engine::assembly::compile(target, source)
        ...
        let tags = match facts.architecture {
            ArchitectureAnalysis::X86 { cpuid, .. } => cpuid,
