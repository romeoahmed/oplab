...
impl LoadPlan {
    /// Plan static ELF64 mappings from program headers using the supplied page size.
    ///
    /// Ordinary section headers and symbols are unnecessary; extended program-header
    /// counts use section-zero metadata. Page padding and storage beyond `p_filesz` are zero.
    ///
    /// # Errors
    ///
    /// Rejects malformed ELF, target mismatch, unsupported runtime requirements,
    /// invalid page size/geometry/entry, permission conflicts and excessive allocation.
    /// Range and allocation budgets are checked before copying segment contents.
